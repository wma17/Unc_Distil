...
and full uncertainty decomposition (TU / EU / AU).
...
Figures:
    ...
    8. Teacher vs Student decomposition on clean CIFAR-10

Usage:
    python plot_eu.py --save_dir ./checkpoints --gpu 0
...
from evaluate_student import (
    SEEN_OOD, UNSEEN_OOD, CIFAR10_MEAN, CIFAR10_STD, EPS,
...
COLORS = {
    "Clean CIFAR-10": "#2ecc71",
    "Shifted CIFAR-10": "#f39c12",
    "SVHN": "#3498db",
    "CIFAR-100": "#2980b9",
    "MNIST": "#e74c3c",
    "FashionMNIST": "#c0392b",
...
def collect_all_eu(model, data, args, device):
    """Collect teacher EU/TU/AU, student EU, and student probs for every dataset.

    Returns dict: name -> {
        "teacher_eu": np.array,
        "teacher_tu": np.array or None,
        "teacher_au": np.array or None,
        "student_eu": np.array,
    ...
    # Corrupted CIFAR-10 test (aggregate)
...
# ---------------------------------------------------------------------------
# Figure 1: Teacher vs Student EU scatter
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Figure 2 & 3: EU distribution histograms
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Figure 5: Uncertainty decomposition — stacked bars (AU + EU = TU)
# ---------------------------------------------------------------------------

def plot_decomposition_bars(all_eu, out_dir):
    """Stacked bar chart: AU (bottom) + EU (top) = TU, teacher vs student side by side."""
    names = list(all_eu.keys())
    n = len(names)
    ...
    for name in names:
        d = all_eu[name]
        # Student: TU = H[probs], EU = eu_head, AU = TU - EU
        ...
        # Teacher: use cached TU/AU when available
        t_eu = d["teacher_eu"]
    ...
    for i in range(n):
        if teacher_au[i] is not None:
            ...
            ax.text(x[i] - width/2, teacher_eu[i] + 0.005, "EU\nonly",
    ...
    ax.set_title("Uncertainty Decomposition: TU = AU + EU\n"
                 "Teacher (left, blue) vs Student (right, red); ▼ = TU", fontsize=13)
...
# ---------------------------------------------------------------------------
# Figure 6: Student TU / EU / AU distributions per dataset
# ---------------------------------------------------------------------------

def plot_student_decomposition_dists(all_eu, out_dir):
    """Three-panel figure: TU, EU, AU density overlays across datasets."""
...
# ---------------------------------------------------------------------------
# Figure 7: Teacher TU / EU / AU distributions across all datasets
# ---------------------------------------------------------------------------

def plot_teacher_decomposition_dists(all_eu, out_dir):
    """Three-panel figure: teacher TU, EU, AU density overlays across datasets."""
    fig, axes = plt.subplots(1, 3, figsize=(18, 5))
...
# ---------------------------------------------------------------------------
# Figure 8: Teacher vs Student decomposition on clean CIFAR-10
# ---------------------------------------------------------------------------

def plot_teacher_vs_student_decomposition(all_eu, out_dir):
    """Teacher vs Student TU/EU/AU on clean CIFAR-10."""
    d = all_eu.get("Clean CIFAR-10")
    if d is None or d["teacher_tu"] is None:
        print("  Skipping teacher-vs-student decomposition (no teacher TU for clean data)")
        return
    ...
    for ax, t_vals, s_vals, bins, label in zip(
            axes,
            [t_tu, t_eu, t_au], [s_tu, s_eu, s_au],
            [bins_tu, bins_eu, bins_au],
            ["TU", "EU", "AU"]):
    ...
    fig.suptitle("Clean CIFAR-10 Test — Teacher vs Student Decomposition", fontsize=14, y=1.02)
    fig.tight_layout()
...
def main():
    parser = argparse.ArgumentParser(description="Plot EU distributions, correlations, and decomposition")
