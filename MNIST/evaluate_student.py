"""
Comprehensive evaluation of the distilled MNIST student model.
...
Reports:
    ...
    4. OOD detection AUROC     — teacher EU vs student EU vs baselines
       Seen OOD  (Phase 2):    FashionMNIST, Omniglot
       Unseen OOD (held out):  EMNIST-Letters, CIFAR-10 (gray), SVHN (gray)
    5. Uncertainty decomposition (TU / EU / AU)
    6. Decomposed OOD detection
    7. Single-model baseline — one ensemble member's entropy & max-prob for OOD
...
SEEN_OOD = [
    ...
    ("omniglot", "omniglot_eu", "Omniglot"),
]
UNSEEN_OOD = [
    ("emnist_letters", "emnist_letters_eu", "EMNIST-Letters"),
    ("cifar10", "cifar10_eu", "CIFAR-10"),
    ("svhn", "svhn_eu", "SVHN"),
]
...
def get_test_loader(data_dir, batch_size=256, num_workers=4):
    ...
    ds = datasets.MNIST(root=data_dir, train=False, download=False, transform=transform)
    return DataLoader(ds, batch_size=batch_size, shuffle=False,
                      num_workers=num_workers, pin_memory=True)


def get_ood_loader(ood_name, data_dir, batch_size=256, num_workers=4):
    gray_to_3ch = transforms.Compose([
    ...
    loaders = {
        "fashionmnist": lambda: datasets.FashionMNIST(
            root=data_dir, train=False, download=False, transform=gray_to_3ch),
        "omniglot": lambda: datasets.Omniglot(
            root=data_dir, background=False, download=False, transform=gray_to_3ch),
        "emnist_letters": lambda: datasets.EMNIST(
            root=data_dir, split="letters", train=False, download=False, transform=gray_to_3ch),
        "cifar10": lambda: datasets.CIFAR10(
            root=data_dir, train=False, download=False, transform=rgb_transform),
        "svhn": lambda: datasets.SVHN(
            root=os.path.join(data_dir, "svhn"), split="test",
            download=False, transform=rgb_transform),
    }
    if ood_name not in loaders:
        raise ValueError(f"Unknown OOD: {ood_name}. Available: {list(loaders.keys())}")
    ds = loaders[ood_name]()
    return DataLoader(ds, batch_size=batch_size, shuffle=False,
                      num_workers=num_workers, pin_memory=True)
...
def predict_student(model, loader, device):
    ...
    for batch in loader:
        ...
        all_eu.append(eu.clamp(min=0).cpu())
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate distilled MNIST student")
    ...
    # 1b. Calibration Metrics (ECE-15, NLL)
    ...
    print(f"\n{'='*60}")
    print(f"  1b. Calibration Metrics (clean MNIST test)")
    print(f"{'='*60}")
    ...
    print(f"  {'Clean MNIST test':<30} {r_p:>10.4f} {r_s:>10.4f} "
    ...
    if unseen_available:
        print(f"\n{'='*60}")
        print(f"  4b. OOD Detection — UNSEEN OOD")
        print(f"{'='*60}")
    ...
    if shifted_stu_eu is not None and all_ood_data:
        print(f"\n{'='*60}")
        print(f"  4c. OOD Detection — Shifted MNIST vs OOD")
        print(f"{'='*60}")
        for ood_id, (s_eu, t_eu, s_probs, _ttu, _tau, dname) in all_ood_data.items():
            print_ood_table("Shifted MNIST", dname,
    ...
    if shifted_stu_eu is not None:
        ...
        print(f"{'='*60}")
        print_ood_table("Clean MNIST", "Shifted MNIST",
    ...
    print(f"\n{'='*60}")
    print(f"  5. Uncertainty Decomposition (Clean MNIST test)")
    print(f"{'='*60}")
    ...
    if all_ood_data:
        print(f"\n{'='*60}")
        print(f"  6. OOD Detection: MNIST vs OOD — Decomposed Uncertainties")
        print(f"{'='*60}")
        print(f"  {'Dataset':<18} {'Type':<7} "
    ...
    if single_model is not None and all_ood_data:
        ...
        print(f"\n  {'Dataset':<18} {'Type':<7} {'Entropy':>10} {'1-MaxProb':>10}")
        print(f"  {'-'*50}")
        ...
        for ood_id, (s_eu, t_eu, s_probs, _ttu, _tau, dname) in all_ood_data.items():
            ...
            try:
                ...
                print(f"  {dname:<18} {ood_type:<7} SKIPPED ({e})")
                continue
            ...
            print(f"  {dname:<18} {ood_type:<7} {a_ent:>10.4f} {a_mp:>10.4f}")
        ...
        if shifted_stu_probs is not None:
            ...
            print(f"  {'Shifted MNIST':<18} {'shift':<7} {a_ent:>10.4f} {a_mp:>10.4f}")

    # ======================================================================
    # 8. Selective Prediction — AURC & Sparsification
    # ======================================================================
    print(f"\n{'='*60}")
    print(f"  8. Selective Prediction (AURC) — Clean MNIST test")
    print(f"{'='*60}")
